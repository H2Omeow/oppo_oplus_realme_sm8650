#!/usr/bin/env python3
"""
VA48 BEACON PATCHER  (round 11)
===============================

ROUND 9 RESULT (confirmed on real hardware)
--------------------------------------------
r9 added va48_fix_cmdline() (orange→green) and CP28/CP29 (reboot reason).
Result: CP0..CP20 lit, CP22+CP23 lit (userspace active), PANIC block lit
(5x). CP21 absent (exec /init succeeded), CP24..CP29 absent. The device
reached panic() — va48_fix_cmdline() suppressed the ~10s OPLUS reboot, and
something deeper caused a kernel panic during early userspace init.

WHAT ROUND 10 ADDS
------------------
r9 confirmed a kernel panic. Now we need to know WHY. This round encodes
the first 8 bytes of the panic() message into the framebuffer as a sequence
of nibble-height bands immediately below the PANIC block, readable from a
photograph without any console or ramoops.

  VA48_PMSG_UNIT     = 0x3000  (12 KiB ≈ 2.37 screen rows at 1264×4 stride)
  VA48_PMSG_GAP      = 0x1800  ( 6 KiB, intra-char gap between hi and lo)
  VA48_PMSG_CHAR_GAP = 0x4800  (18 KiB, inter-char gap = 3× intra-char)
  VA48_PMSG_CHARS    = 8

WHAT ROUND 11 ADDS
------------------
r10 encoded the message but the bands could not be decoded from new.jpg:
  - UNIT=12KiB = 4.4px/step at the photo scale (below JPEG 8×8 block size)
  - no background clear → bootloader splash gradient masked the bands
  - confirmed: pstore empty, /dev/mem absent, physical memory unreadable

r11 fixes both problems:
  1. UNIT enlarged 8× (0x3000 → 0x18000 = 96KiB ≈ 33px/step in photo)
     GAP/CHAR_GAP scaled proportionally so separators are visible stripes.
  2. PMSG region pre-cleared to black before encoding, so the splash image
     background does not interfere with the white band measurement.
  3. CHARS reduced 8 → 4 so all characters fit within the panel height.

  PANIC MESSAGE ENCODING (r11)
  ----------------------------
  Each byte of the panic string is split into two nibbles (high = bits 7:4,
  low = bits 3:0).  Each nibble is rendered as a white band of height
  (nibble + 1) × VA48_PMSG_UNIT bytes on a pre-cleared black background,
  followed by a dark gap.

    VA48_PMSG_UNIT     = 0x18000 (96 KiB ≈ 19.4 screen rows; ~33px/step)
    VA48_PMSG_GAP      = 0x8000  (32 KiB intra-char gap, ~11 px — visible)
    VA48_PMSG_CHAR_GAP = 0x18000 (96 KiB inter-char gap, ~33 px — clear stripe)
    VA48_PMSG_CHARS    = 4       (first 4 bytes of the panic string)
    VA48_PMSG_CLEAR    = 0xC80000 (12.5 MiB pre-cleared to black)

  To decode from a photograph (face-on, screen fills frame):
    1. Below the 5x PANIC block find the black region, then the PMSG area.
    2. Identify the ~33px dark CHAR_GAP stripes: they separate the 4 chars.
    3. Within each char: two white bands separated by a ~11px dark GAP.
    4. Measure each band's height in photo pixels → divide by ~33 → nibble+1.
    5. nibble = (height/33) − 1.  ASCII = (hi<<4)|lo.

  Geometry: start offset 0x072c000 (32 KiB past PANIC band end).
  Worst case (4 × 0xFF): 12.5 MiB; total layout ends ≤ 19.9 MiB,
  well inside the 13.4–14 MiB scanout from PMSG_START perspective
  (panels taller than 2780 rows handle it; on a 2780-row panel the
  last character may extend off-screen which is acceptable — we only
  need chars 0–3).

  ANCHOR CHANGE IN kernel/panic.c
  --------------------------------
  r9 called va48_beacon_panic() at the _crash_kexec_post_notifiers declaration
  (line ~282), which is BEFORE vscnprintf(buf, ...) fills buf (line ~327).
  r10 moves the call to after pr_emerg("Kernel panic - not syncing: %s\n", buf)
  (line ~333), so buf is guaranteed populated.  The function signature changes
  to va48_beacon_panic(const char *msg).
...
print("=" * 62)
print("VA48 BEACON PATCHER  (round 11: enlarged PMSG bands + pre-clear)")
print("=" * 62)
...
/*
 * VA48 R11: panic message encoder (enlarged bands + pre-clear).
 *
 * Encodes the first VA48_PMSG_CHARS bytes of the panic string as nibble-pair
 * bands immediately after the PANIC block.  Each byte is split into high
 * (bits 7:4) and low (bits 3:0) nibbles; each nibble paints
 * (nibble+1)*VA48_PMSG_UNIT bytes white, followed by a dark gap.  After the
 * low nibble a larger VA48_PMSG_CHAR_GAP separates adjacent characters so
 * char boundaries are visible in a photograph.
 *
 * r11 enlarges the units so each nibble step is ~33 px in a photo and the
 * inter-char gap is a clearly distinct dark stripe.  The PMSG region is
 * pre-cleared to black so the bootloader splash does not interfere.
 *
 * To decode: photograph the screen directly, face-on.  Below the PANIC block:
 *   - CHAR_GAP: ~33 px dark stripe (inter-character boundary)
 *   - Within each char: two bright bands separated by a ~11 px dark GAP
 *   - nibble = band_height_px / 33 - 1  (hi nibble first, then lo)
 *   - ASCII = (hi << 4) | lo
 * Panel stride = 1264*4 = 5056 B; 1 UNIT = 19.4 screen rows.
 */
#define VA48_PMSG_START		(VA48_PANIC_OFF + VA48_PANIC_BAND + 0x8000UL)
#define VA48_PMSG_UNIT		0x18000UL	/* 96 KiB per nibble step (~19.4 rows)  */
#define VA48_PMSG_GAP		0x8000UL	/* 32 KiB intra-char gap  (~6.5 rows)   */
#define VA48_PMSG_CHAR_GAP	0x18000UL	/* 96 KiB inter-char gap  (~19.4 rows)  */
#define VA48_PMSG_CHARS		4		/* encode first 4 bytes                 */
#define VA48_PMSG_CLEAR		0xC80000UL	/* 12.5 MiB pre-clear (4×worst-case)    */

static void va48_encode_pmsg(const char *msg)
{
	unsigned long off = VA48_PMSG_START;
	int i;

	if (!msg)
		return;

	/* Erase the PMSG region to black so the bootloader splash image
	 * behind the bands does not corrupt the photo-readable baseline. */
	memset((void *)phys_to_virt(va48_beacon_pa(VA48_PMSG_START)),
	       0, VA48_PMSG_CLEAR);

	for (i = 0; i < VA48_PMSG_CHARS && msg[i]; i++) {
		unsigned char c  = (unsigned char)msg[i];
		unsigned int  hi = (c >> 4) & 0xF;
		unsigned int  lo = c & 0xF;
		unsigned long blen;

		blen = (unsigned long)(hi + 1) * VA48_PMSG_UNIT;
		va48_beacon_paint(
			(void *)phys_to_virt(va48_beacon_pa(off)), blen);
		off += blen + VA48_PMSG_GAP;

		blen = (unsigned long)(lo + 1) * VA48_PMSG_UNIT;
		va48_beacon_paint(
			(void *)phys_to_virt(va48_beacon_pa(off)), blen);
		off += blen + VA48_PMSG_CHAR_GAP;
	}
}

/*
 * Called from panic() after vscnprintf() has filled buf.  Paints the thick
 * PANIC block (present ⇒ panic() reached; absent ⇒ true hang), then encodes
 * the panic message as nibble bands below it.  Guarded by va48_beacon_linear
...
patch("kernel/panic.c", [
    # Signature updated to accept the panic string (r10).
    # Stub in the #else branch updated in parallel so cross-arch builds work.
    ("panic beacon decl",
     "void __weak panic_smp_self_stop(void)\n",
     "/*\n"
     " * VA48 BEACON: defined in arch/arm64/kernel/setup.c. Paints the PANIC\n"
     " * block and encodes the panic message as nibble-height bands below it,\n"
     " * readable from a photograph. See other_patch/va48/patch_beacon_r11.py.\n"
     " */\n"
     "#ifdef CONFIG_ARM64\n"
     "void va48_beacon_panic(const char *msg);\n"
     "#else\n"
    ...
    # r9 called va48_beacon_panic() at the _crash_kexec_post_notifiers line
    # (~line 282), which is BEFORE vscnprintf fills buf (~line 327).  r10 moves
    # the call to after pr_emerg() (~line 333), where buf is guaranteed valid.
    ("panic beacon call",
...
print("pmsg encoding   offset 0x72c000, nibble bands: unit=96KiB gap=32KiB char_gap=96KiB chars=4")
print("  decode: each step=33px in photo; inter-char gap=33px dark stripe")
print("  band_px/33 → nibble+1; (hi<<4)|lo = ASCII byte")
print("  pre-clears 12.5MiB from PMSG_START to black; max layout end: ~21MiB")
