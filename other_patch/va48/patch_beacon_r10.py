...
photograph without any console or ramoops.
  ...
  (nibble + 1) × VA48_PMSG_UNIT bytes, followed by a dark gap.  After the
  low nibble there is a larger inter-character gap so adjacent char pairs
  are visually distinct.
    ...
    VA48_PMSG_CHARS    = 8       (first 8 bytes of the panic string)

  To decode from a photograph:
    1. Below the 5x PANIC block, measure each bright band's height in rows.
    2. Divide by 2.37 rows/unit and round to nearest integer → band height H.
    3. nibble = H − 1.
    4. Pair nibbles: (hi<<4)|lo → ASCII byte.
    5. Char boundary: the gap after lo nibble is ~7.1 rows (3×), vs ~2.4 rows
       (1×) between hi and lo within the same char.

  Geometry: start offset 0x072c000 (32 KiB past PANIC band end).
  Worst case (8 × 0xFF): 3.19 MiB; total layout ends ≤ 10.7 MiB,
  well inside the 13.4 MiB scanout (1264×2780×4).
...
print("=" * 62)
print("VA48 BEACON PATCHER  (round 10: panic message nibble encoding)")
print("=" * 62)
...
/*
 * VA48 R10: panic message encoder.
 *
 ...
 *
 * To decode: measure each bright band's height in raw framebuffer bytes (H),
 * compute nibble = H/VA48_PMSG_UNIT - 1, pair as (hi<<4)|lo → ASCII.
 * In the photo: 1 unit ≈ 2.37 screen rows (panel stride = 1264*4 = 5056 B).
 * Intra-char gap ≈ 1.19 rows (1×), inter-char gap ≈ 3.56 rows (3×).
 */
#define VA48_PMSG_START		(VA48_PANIC_OFF + VA48_PANIC_BAND + 0x8000UL)
#define VA48_PMSG_UNIT		0x3000UL	/* 12 KiB per nibble step     */
#define VA48_PMSG_GAP		0x1800UL	/*  6 KiB intra-char gap      */
#define VA48_PMSG_CHAR_GAP	0x4800UL	/* 18 KiB inter-char gap (3×) */
#define VA48_PMSG_CHARS		8		/* encode first 8 bytes       */
...
patch("kernel/panic.c", [
    ...
    ("panic beacon decl",
     ...
     " * readable from a photograph. See other_patch/va48/patch_beacon_r10.py.\n"
     " */\n"
     "#ifdef CONFIG_ARM64\n"
     "void va48_beacon_panic(const char *msg);\n"
     "#else\n"
...
print("pmsg encoding   offset 0x72c000, nibble bands: unit=12KiB gap=6KiB char_gap=18KiB")
print("  decode: band_rows/2.37 → band_height; nibble=height-1; (hi<<4)|lo = ASCII")
print("  max layout end: ~10.7 MiB (1264×2780×4 = 13.4 MiB scanout)")
