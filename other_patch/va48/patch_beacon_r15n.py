#!/usr/bin/env python3
"""
VA48 BEACON PATCHER r15n (enhanced BoringSSL diagnostics)
==========================================================

CHANGES VS r15
--------------
1. Enhanced BoringSSL exit logging:
...
2. Enhanced BoringSSL sigfault logging:
   - Same detailed register and mm dump
   - Show fault type and address decode

3. NO panic() on boringssl-self-check-failed:
   - Remove panic() call from reboot.c RESTART2 handler
   - Just log reason and restart normally
   - This lets Android init retry and we see all evidence

4. All other beacon checkpoints (CP0-CP29) unchanged from r15.
"""
...
SETUP_FN_NEW = r"""/*
 * VA48 BEACON r15n (enhanced BoringSSL diagnostics)
 *
 * KEY CHANGES vs r15:
 *  1. va48_beacon_exit: decode exit code, show all registers, mm layout, VMAs
				  ...
				  const char *str)
{
	...
	if (regs) {
		...
		/* For BoringSSL, show more registers and mm layout */
		if (bssl) {
			struct mm_struct *mm;
			...
			snprintf(msg, sizeof(msg), "X0=%016lx X1=%016lx X29=%016lx X30=%016lx",
				 regs->regs[0], regs->regs[1], regs->regs[29], regs->regs[30]);
			pr_emerg("VA48 BSSL %s\n", msg);
			va48_log_late(msg);
...
void va48_beacon_exit(long code)
{
	...
	if (regs) {
		snprintf(msg, sizeof(msg),
			 "  pc=%016lx sp=%016lx pstate=%016lx",
...
patch("kernel/reboot.c", [
    ...
    ("CP29 at RESTART2 case",
     ...
     "\t\t/*\n"
     "\t\t * VA48 r15n: do NOT panic on boringssl-self-check-failed.\n"
     "\t\t * Just log it to screen and restart normally. This lets\n"
     "\t\t * Android init retry the process, and we'll capture detailed\n"
     "\t\t * diagnostics from va48_beacon_exit() and va48_beacon_sigfault().\n"
     "\t\t */\n"
     "\t\tkernel_restart(buffer);\n"),
])
